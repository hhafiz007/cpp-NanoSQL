...
void printIndexLeafPage(std::vector<char> &database_file , unsigned short rootPage,string indexValue) {
    ...
    for(unsigned long i = 0 ; i*4096 < database_file.size();i+=1)
    {
    unsigned long pageStart = i*4096;

    unsigned short pageType = static_cast<unsigned char>(database_file[916588544]);
    
      
     std::cout <<" The page type is" << pageType<<endl;
